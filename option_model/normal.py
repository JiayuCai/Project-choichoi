# -*- coding: utf-8 -*-
"""
Created on Fri Nov  3 08:28:08 2017
...
def get_d(strike, spot, texp, vol, intr=0.0, divr=0.0):
...
#function to calculate implied volatility through brent method
def impvol_brentq( price_in, strike, spot, texp, divr=0, intr=0, cp_sign=1):
    div_fac = np.exp(-texp * divr)
    disc_fac = np.exp(-texp * intr)
    forward = spot/disc_fac*div_fac
    strike = np.array(strike).reshape(-1)
    price_in = np.array(price_in).reshape(-1)
    ...
    if(int_val > price_in).any():
        raise ValueError('Option value is lower than intrinsic value', price_in, int_val) 
    vol1=np.zeros(len(strike))
    for i in range(len(strike)):
        iv_func = lambda _vol: \
            price(strike[i], forward, texp, _vol, cp_sign=cp_sign) - price_fwd[i]#为什么是price_fwd不是price？

        vol1[i] = sopt.brentq(iv_func, 0, price_straddle[i]*np.sqrt(np.pi/2/texp))#函数要求f(a)和f(b)符号相反，这里好像不符合？
    if len(strike)==1:
        vol1 = vol1[0]
    return vol1


#%%calculate implied volatility through Chebyshev polynomials
def impvol_chebyshev( price_in, strike, spot, texp, divr=0, intr=0, cp_sign=1):
    div_fac = np.exp(-texp * divr)
    disc_fac = np.exp(-texp * intr)
    forward = spot/disc_fac*div_fac
    strike = np.array(strike).reshape(-1)
    ...
    #xs denotes eta in h(eta) for in-the-money option (split the [0,1] interval into 4 sub-intervals), and is used to choose coefficients for a given eta input
    xs = [0, 0.4217386245727539 , 0.55084228515625 , 0.7421875 , 1.0]

    #Ci is a 4 by 12 matrix, and each row corresponds to each interval in xs
    Ci = [[1.14576494380569338e+00 , 4.93894933953413273e-01 , 9.50848289426610774e-03 , -5.06355533662371324e-03 , 
          ...
          4.04566190562942834e-07 , -2.21891236355190701e-08 , 1.36269939039333248e-09 , -8.84037949667989474e-11 , 
          5.87957127773745941e-12 , -3.94308493169714526e-13 , 2.63924286682272227e-14 , -1.74264358622528222e-15]]
    
    
    #multipler is used to calculate beta for out-of-money option (split the interval into 24 sub-intervals)
    multiplier = [0, 0.05263201943695479 , 0.21052807774781915 , 0.5614082073275177, 0.816593756112753, 
                  1.1877727361640043 , 1.6525533720542671 , 2.1152683162294617 , 2.7075434447737114 , 
                  3.5363832748064796 , 4.618949583420708 , 6.03291374161072 , 7.960958339445074 , 
                  10.505182138649168 , 13.791418602534293 , 18.10565724230143 , 23.769478225790593 , 
                  31.61031909507736 , 42.25713675373266 , 57.16156940002939 , 77.94067518725711 , 
                  107.4899008643115 , 149.65283274650574 , 210.7177734375 , 300]

    multiplier_copy = multiplier.copy()
    multiplier_copy.reverse()
    beta = [x* np.log(10) for x in multiplier_copy]

    #embeta is used to choose beta for a given z
    embeta = [np.exp(-x) for x in beta]
    
    #Co is a 24 by 12 matrix, and  each row corresponds to each interval in embeta
        ...
        [1.61575635045128161e+03 , 1.40084371997021321e+02 , 2.26962330718600939e-02 , -1.31117114957347044e-03 ,
         ...
         [1.14244577839507951e+03 , 9.65746640508781269e+01 , 2.14832644611544811e-02 , -1.20564164636571738e-03 ,
          ...
          [8.14221183081155800e+02 , 6.75408915344469563e+01 , 2.05749962283125476e-02 , -1.12765405950729131e-03 ,
           6.93590677615677035e-05 , -4.54215064646345091e-06 , 3.09391570448861576e-07 , -2.16498374498513000e-08 ,
           1.54488140956553519e-09 , -1.11884629996816779e-10 , 8.19721227056071115e-12 , -6.02828138980731756e-13],
           [5.84426844422268118e+02 , 4.73595395993986656e+01 , 1.94939514080227326e-02 , -1.03704438926066275e-03 ,
            ...
            1.21122172934082963e-09 , -8.48498666448666160e-11 , 6.01114446148331094e-12 , -4.27486159462142734e-13],
            [4.22034946751639438e+02 , 3.38390285345843012e+01 , 1.89105365344666258e-02 , -9.87111358507962184e-04 ,
             ...
             [3.06268330750858979e+02 , 2.40471850984675086e+01 , 1.79105848598168256e-02 , -9.05304056844595767e-04 ,
              ...
              [2.23005332604247087e+02 , 1.75862405473772547e+01 , 1.77868230923009969e-02 , -8.89981137147263465e-04 ,
               ...
               7.20946540939965004e-10 , -4.65702138075843620e-11 , 3.03604866667469958e-12 , -1.98471412648994867e-13],
               [1.62667972355314561e+02 , 1.25851758168342602e+01 , 1.67593410462559926e-02 , -8.06865964259929533e-04 ,
                ...
                5.20470929477186976e-10 , -3.19626959072382770e-11 , 1.97822139293063986e-12 , -1.22657622803902416e-13],
                [1.18566493530120553e+02 , 9.46659775809801829e+00 , 1.73770992840480074e-02 , -8.42034820952365952e-04 ,
                 ...
                 5.34450977458847880e-10 , -3.24729666711432765e-11 , 1.98391152291943098e-12 , -1.21158736934964368e-13],
                 [8.54623767868520474e+01 , 7.08648876487567403e+00 , 1.80734494773889948e-02 , -8.79158514673574593e-04 ,
                  4.71032350485270146e-05 , -2.64751380023519790e-06 , 1.52833265837117697e-07 , -8.96059077944794021e-09 ,
                  5.30015695740678664e-10 , -3.14882916760790340e-11 , 1.87290244808356351e-12 , -1.10860466510251981e-13],
                  [6.05832566290490249e+01 , 5.35345537208291944e+00 , 1.95356468722009541e-02 , -9.66658570039372056e-04 ,
                   5.22578043412872984e-05 , -2.94286978307513055e-06 , 1.69063278706094856e-07 , -9.79612142108334273e-09 ,
                   5.68362021009136972e-10 , -3.28389051927897973e-11 , 1.88036701042893552e-12 , -1.05856918363037906e-13],
                   [4.20438717065676855e+01 , 3.91746142694269928e+00 , 2.02750971539916586e-02 , -9.91367660155324879e-04 ,
                    ...
                    [2.87418835904141439e+01 , 2.73641102606634634e+00 , 1.92845978996126176e-02 , -8.81921217829938252e-04 ,
                     4.27034371821736626e-05 , -2.08572918672709561e-06 , 1.00044714573384490e-07 , -4.59988790004481300e-09 ,
                     ...
                     [1.93543305481815686e+01 , 1.95906208202102228e+00 , 1.93135538600447271e-02 , -8.34150301839725063e-04 ,
                      ...
                      [1.27082048601427715e+01 , 1.36630289513067926e+00 , 1.85821399756413361e-02 , -7.24350366367430766e-04 ,
                       2.72372921197427225e-05 , -8.83395001301227846e-07 , 1.82450391048389667e-08 , 4.19109646214676921e-10 ,
                       ...
                       [8.25768664193382484e+00 , 8.64284873544575172e-01 , 1.44356983322932973e-02 , -4.40953083333075742e-04 ,
                        ...
                        [5.37399664023837076e+00 , 5.80508797906514662e-01 , 1.22255955381050784e-02 , -2.90168333061641029e-04 ,
                         3.97210055405662473e-06 , 1.13122593045570815e-07 , -1.19881618657853447e-08 , 5.62804550979071614e-10 ,
                         ...
                         [3.26687382369443746e+00 , 4.69794428067438763e-01 , 1.60977402884881701e-02 , -3.17785406710487833e-04 ,
                          -2.78170698806923216e-06 , 6.51860031800023276e-07 , -3.31395243703917697e-08 , 6.51855655888523646e-10 ,
                          ...
                          [1.78792847895452378e+00 , 2.74109133572780550e-01 , 1.20722645379343248e-02 , -8.29580374604372054e-05 ,
                           -8.56476604286481894e-06 , 4.31400725863084110e-07 , -3.36823221341919302e-09 , -7.00281644535294660e-10 ,
                           ...
                           [9.89202680545164026e-01 , 1.31616375453412121e-01 , 5.76124504188479415e-03 , 2.57095969945979021e-05 ,
                            -3.38132571124674654e-06 , 3.80592728338321150e-08 , 2.70328591323312130e-09 , -1.13648818350287395e-10 ,
                            ...
                            [4.97231967506140016e-01 , 1.10588522091192326e-01 , 9.28845342365719299e-03 , 2.35157347934832338e-04 ,
                             -1.06786619980406935e-05 , -3.85205611446075403e-07 , 3.05681991112453435e-08 , 4.74449512974709131e-10 ,
                             ...
                             [2.34773761638268075e-01 , 2.82691112989171545e-02 , 1.38574497701665850e-03 , 2.88585623712612742e-05 ,
                              -8.80883333178174867e-08 , -1.33101610268625247e-08 , 1.12451491663278291e-11 , 7.87035584116022092e-12 ,
                              -1.13117181931959606e-14 , -5.15279626949370386e-15 , 1.86890463893239915e-17 , 3.52592989927947130e-18],
                              [1.66410700339358342e-01 , 7.13999302958060519e-03 , 1.28165917685898399e-04 , 1.07103294197533383e-06 ,
    ...
    x = (forward - strike)*cp_sign
        
    ###calculate vol
    #near the money: abs(x)<np.spacing(1)
    vol0=np.where(abs(x)<np.spacing(1),price_fwd*np.sqrt(2*np.pi/texp),0)
    
    #not near the money
    z=np.select([(abs(x)>=np.spacing(1)) & (x>0),(abs(x)>=np.spacing(1)) & (x<=0)],\
                [(price_fwd - x)/x,-price_fwd/x],default=0.1)
    z=np.where(z>=0.2,0.5,z)
    z=z.reshape(1,len(strike))
    minarg = np.array(abs(np.matrix(embeta)-np.matrix(z).T).argmin(axis=1))
    j = np.where(z>np.array(embeta)[minarg].reshape(len(strike),),minarg.reshape(len(strike),),\
                 minarg.reshape(len(strike),)-1)
    
    u=np.where((abs(x)>=np.spacing(1)) &(z < 0.2), -(np.log(z)+np.array(beta)[j+1])/\
               (np.array(beta)[j]-np.array(beta)[j+1]),0)
    coeff = np.array(Co)[j].reshape(len(strike),12)
    hz = chebyshev(u, coeff, 0, 1)
    vol0 = np.where((abs(x)>=np.spacing(1)) &(z < 0.2), abs(x)/np.sqrt(hz*texp),vol0)
    
    
    z1 = np.select([(abs(x)>=np.spacing(1)) & (z>=0.2) &(x < 0),(abs(x)>=np.spacing(1))\
                   & (z>=0.2) &(x>=0)],[abs (x) / (price_fwd-x),abs (x) / price_fwd],default=z)
    u1 = np.where((abs(x)>=np.spacing(1)) & (z>=0.2),eta (z1, 1e-2),0.1)
    u1=u1.reshape(1,len(strike))

    minarg1 = np.array(abs(np.matrix(xs)-np.matrix(u1).T).argmin(axis=1))
    j1 = np.where(u1>np.array(xs)[minarg1].reshape(len(strike),),minarg1.reshape(len(strike),),\
                 minarg1.reshape(len(strike),)-1)
    
    coeff1 = np.array(Ci)[j1].reshape(len(strike),12)
    hz1 = chebyshev(u1, coeff1, np.array(xs)[j1], np.array(xs)[j1+1])

    vol0=np.select([(abs(x)>=np.spacing(1)) & (z>=0.2) &(x < 0),(abs(x)>=np.spacing(1))\
                   & (z>=0.2) &(x>=0)], [(price_fwd-x)* hz1 / np.sqrt(texp), price_fwd\
                     * hz1 / np.sqrt(texp)],default=vol0)
...
def eta(z, UpLimit):
    eta = np.where(z<UpLimit,1-z*(0.5+z*(1.0/12+z*(1.0/24+z*(19.0/720+z*(3.0/160+z*(863.0/60480\
                +z*(275.0/24192+z*(33953.0/3628800+z*(8183.0/1036800))))))))),-z/np.log(1-z))
    return eta

#Chebyshev polynomials with 11 degree
def chebyshev(x,c,a,b):
    y = (2*x-a-b)/(b-a)
    y2 = 2*y
    b0 = c[:, 11]
    b1 = 0.0
    ...
    b2 = b1; b1 = b0; b0 = y2*b1-b2+c[:, 1]
    value = y*b0 - b1 + 0.5*c[:, 0]
    return value


#%%calculate implied volatility through LFK4
def impvol_LFK4( price_in, strike, spot, texp, divr=0, intr=0, cp_sign=1):
    div_fac = np.exp(-texp * divr)
    disc_fac = np.exp(-texp * intr)
    forward = spot/disc_fac*div_fac
    price_fwd = price_in/disc_fac
    ...
    #Below are coefficients for different intervals of x and z
    ...
    bLFK4 = [0.6409168551974357 , 788.5769356915809 , 445231.8217873989 , 149904950.4316367 , 32696572166.83277 ,
             4679633190389.852 , 420159669603232.9 , 2.053009222143781e+16 , 3.434507977627372e+17 , 
             2.012931197707014e+16 , 644.3895239520736 , 211503.4461395385 , 42017301.42101825 , 
             5311468782.258145 , 411727826816.0715 , 17013504968737.03 , 247411313213747.3]
    ...
    betaStart = -np.log(0.15)
    betaEnd = -np.log(2.2251e-308) #same as realmin() in MATLAB 
    x = (forward-strike)*cp_sign
        
        #if u<0.0091
    def hz1(u):
        
        num = bLFK4[0] + u*( bLFK4[1] +u*( bLFK4[2] +u*( bLFK4[3] +u*( bLFK4[4]\
                   +u*( bLFK4[5] +u*( bLFK4[6] +u*( bLFK4[7] +u*(bLFK4[8] +u* bLFK4[9]))))))))
        den = 1.0 + u*( bLFK4[10] +u*( bLFK4[11] +u*( bLFK4[12] +u*( bLFK4[13]\
                       +u*( bLFK4[14] +u*( bLFK4[15] +u*( bLFK4[16] )))))))
        hz = num / den
        return hz
    
    #if u < 0.088
    def hz2(u):
        num = cLFK4[0] + u*( cLFK4[1] +u*( cLFK4[2] +u*( cLFK4[3] +u*( cLFK4[4]\
                   +u*( cLFK4[5] +u*( cLFK4[6] +u*( cLFK4[7] +u*(cLFK4[8] +u* cLFK4[9] ))))))));
        den = 1.0 + u*( cLFK4[10]+u*( cLFK4[11]+u*( cLFK4[12]+u*( cLFK4[13]+\
                       u*( cLFK4[14]+u*( cLFK4[15]+u*( cLFK4[16])))))));
        hz = num / den 
        return hz
    
    #else
    def hz3(u):
        num = dLFK4[0]+ u*( dLFK4[1]+u*( dLFK4[2]+u*( dLFK4[3]+u*( dLFK4[4]+\
                   u*( dLFK4[5]+u*( dLFK4[6]+u*( dLFK4[7]+u*(dLFK4[8]+u* dLFK4[9]))))))));
        den = 1.0 + u*( dLFK4[10]+u*( dLFK4[11]+u*( dLFK4[12]+u*( dLFK4[13]+\
                       u*( dLFK4[14]+u*( dLFK4[15]+u*( dLFK4[16])))))));
        hz = num / den 
        return hz

    def hz4(u):
        num = aLFK4[0]+ u*( aLFK4[1]+u*( aLFK4[2]+u*( aLFK4[3]+u*( aLFK4[4]+u*\
                   ( aLFK4[5]+u*( aLFK4[6]+u*( aLFK4[7])))))))
    ...
    z=np.select([(abs(x)>=np.spacing(1)) & (x>0),(abs(x)>=np.spacing(1)) & (x<=0)],\
                [(price_fwd - x)/x,-price_fwd/x],default=0.1)
    u=np.where((abs(x)>=np.spacing(1)) &(z < 0.15),-(np.log(z)+betaStart)/(betaEnd-betaStart),0)
    vol0=np.select([(abs(x)>=np.spacing(1)) &(z < 0.15)&(u<0.0091),(abs(x)>=np.spacing(1))\
                 &(z < 0.15)&(u < 0.088), (abs(x)>=np.spacing(1)) &(z < 0.15)&(u >= 0.088)],
                [abs(x)/(np.sqrt(hz1(u)*texp)),abs(x)/(np.sqrt(hz2(u)*texp)),abs(x)/\
                 (np.sqrt(hz3(u)*texp))],default=vol0)
   
    z1 = np.select([(abs(x)>=np.spacing(1)) & (z>=0.15) &(x < 0),(abs(x)>=np.spacing(1))\
                   & (z>=0.15) &(x>=0)],[abs (x) / (price_fwd-x),abs (x) / price_fwd],default=z)
    u1 = np.where((abs(x)>=np.spacing(1)) & (z>=0.15),eta (z1, 1e-2),u)
    
    vol0=np.select([(abs(x)>=np.spacing(1)) & (z>=0.15) &(x < 0),(abs(x)>=np.spacing(1))\
                   & (z>=0.15) &(x>=0)], [(price_fwd-x)* hz4(u1) / np.sqrt(texp), price_fwd\
                     * hz4(u1) / np.sqrt(texp)],default=vol0)
